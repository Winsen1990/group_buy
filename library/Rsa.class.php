...
class Rsa
{
     ...
     * @return mixed return a resource link on success, FALSE on others
     */ 
     ...
     * @param string $private_key 私钥的文件路径
     * @return string|bool 成功时返回签名字符串,其他情况下返回FALSE
     */
     ...
     * @return string|bool 成功时返回加密并且编码完成的数据,其他情况返回FALSE
     */
     ...
     * @param string $public_key 公钥的文件路径
     * @param string $encode 对待解密的数据进行反编码,默认:base64,可选hex、url
     * @return string|bool 成功时返回加密并且编码完成的数据,其他情况返回FALSE
     */
    ...
    private function hex2bin($data)
    {
        ...
        for($i = 0; $i < $len; $i++)
        {
            $re .= chr(hexdec(substr($data, $i * 2, 1)) << 4) | chr(hexdec(substr($data, $i+1, 1)));
        }
